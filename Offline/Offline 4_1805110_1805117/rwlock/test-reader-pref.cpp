...
void *Reader(void* arg)
{
	...
	readerAcquireTime[threadNUmber] = in;
	in++;
	...
	pthread_spin_lock(&spinlock);
	readerReleaseTime[threadNUmber] = in;
	in++;
	pthread_spin_unlock(&spinlock);
...
void *Writer(void* arg)
{
  ...
  // Occupying the Lock
	...
	pthread_spin_lock(&spinlock);
	writerAcquireTime[threadNUmber] = in;
	in++;
	pthread_spin_unlock(&spinlock);
  ...
  usleep(10000);
	...
	pthread_spin_lock(&spinlock);
	writerReleaseTime[threadNUmber] = in;
	in++;
	pthread_spin_unlock(&spinlock);
...
int main(int argc, char *argv[])
{
	...
	// check if writer excited immediately
	for (int i = 0; i < write_num_threads; i++)
